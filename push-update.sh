...
updates_dir=/data/protonplus_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have protonplus-VERSION-DATE-TYPE-* as format"
...
# Assume protonplus-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.protonplus.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.protonplus.updater/databases/updates.db" \
